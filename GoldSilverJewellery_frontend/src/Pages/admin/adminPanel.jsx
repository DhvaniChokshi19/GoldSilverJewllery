...
function Toast({ toast }) {
  ...
  return (
    <div className={`fixed top-5 right-5 z-50 px-5 py-3 rounded-xl text-sm shadow-lg border transition-all
...
function CollectionsPanel({ onSelect, showToast }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) { showToast(editingId ? "Collection updated!" : "Collection created!"); cancel(); load(); }
  ...
  const toggleVis = async (id) => {
    try {
      ...
      if (data.success) { setList(p => p.map(c => c._id === id ? { ...c, isVisible: data.data.isVisible } : c)); showToast("Visibility updated"); }
    } catch { showToast("Failed", "error"); }
  };

  const del = async (id) => {
    if (!window.confirm("Delete this collection AND all its categories & products?")) return;
    try {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs mt-0.5 tracking-wide" style={{ color: "#8b6b4a" }}>Gold, Silver, Platinum…</p>
        </div>
        <button onClick={openNew} className="px-4 py-2 rounded-xl text-sm font-semibold text-white hover:opacity-90 active:scale-95 transition" style={{ background: "#7c4a1e" }}>+ New Collection</button>
      </div>

      {showForm && (
        <div className="bg-white rounded-2xl shadow-md border border-amber-100 p-6 mb-6">
          <h3 className="font-semibold mb-5" style={{ color: "#2c1a0e" }}>{editingId ? "Edit Collection" : "New Collection"}</h3>
      ...
      {loading ? <Spinner /> : (
        <div className="grid grid-cols-2 gap-4">
          {list.map((col, i) => {
            ...
            return (
              <div key={col._id} className={`${s.bg} rounded-2xl p-5 flex flex-col gap-4 shadow-sm`}>
                <div className="flex items-center justify-between">
                  <p className={`font-bold text-xl ${s.text}`}>{col.name}</p>
                  <Badge visible={col.isVisible} onToggle={() => toggleVis(col._id)} />
                </div>
                <div className="flex gap-2 flex-wrap">
                  <button onClick={() => onSelect(col)} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-white/80 hover:bg-white text-amber-900 transition shadow-sm">📂 Manage Categories</button>
          ...
          {list.length === 0 && (
            <div className="col-span-2 text-center py-16 text-amber-400">
...
function CategoriesPanel({ collection, onSelect, onBack, showToast }) {
  const EMPTY = { name: "", label: "", bgColor: "bg-amber-100", textColor: "text-amber-900", isVisible: true, order: 0 };
  const [list, setList]           = useState([]);
  const [loading, setLoading]     = useState(true);
  const [showForm, setShowForm]   = useState(false);
  const [editingId, setEditingId] = useState(null);
  ...
  const openEdit = (c) => {
    setForm({ name: c.name, label: c.label, isVisible: c.isVisible, order: c.order });
    setImageFile(null); setPreview(""); setExisting(c.imageUrl || ""); setEditingId(c._id); setShowForm(true);
  };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const method = editingId ? "put" : "post";
      const { data } = await axiosInstance[method](url, fd);
      if (data.success) { showToast(editingId ? "Category updated!" : "Category created!"); cancel(); load(); }
      else showToast(data.message, "error");
    } catch (err) { showToast(err.response?.data?.message || "Error", "error"); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        <div>
          <h2 className="text-2xl font-normal flex items-center gap-2" style={{ color: "#2c1a0e" }}>
            <span className="text-sm font-semibold px-2 py-0.5 rounded-lg bg-amber-100 text-amber-800">{collection.name}</span>
            <span>Categories</span>
          </h2>
          <p className="text-xs mt-0.5" style={{ color: "#8b6b4a" }}>Chains, Earrings, Bangles, Tikka, Payal…</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl shadow-md border border-amber-100 p-6 mb-6">
          <h3 className="font-semibold mb-5" style={{ color: "#2c1a0e" }}>{editingId ? "Edit Category" : `New Category under "${collection.name}"`}</h3>
          <form onSubmit={onSubmit} className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <Label text="Storefront Card Preview" />
        ...
        <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-amber-100">
          <table className="w-full text-sm">
            ...
            <tbody>
                ...
                <tr key={cat._id} className={`border-t border-amber-50 hover:bg-amber-50/40 transition ${!cat.isVisible ? "opacity-50" : ""}`}>
                  <td className="px-5 py-3">
                    <div className='rounded-xl flex items-center gap-2 px-3 py-2 w-36'>
                      ...
                      <div className={cat.textColor}>
                        <p className="text-[10px] opacity-70 leading-none">{cat.label}</p>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex gap-2">
                      <button onClick={() => onSelect(cat)} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-amber-50 border border-amber-200 text-amber-700 hover:bg-amber-100 transition">📦 Products</button>
              ...
              {list.length === 0 && <tr><td colSpan={5} className="text-center py-14 text-amber-300">No categories yet.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
...
// ── PRODUCTS ──────────────────────────
function ProductsPanel({ collection, category, onBack, showToast }) {
  ...
  const load = async () => {
    try {
      const { data } = await axiosInstance.get(`/api/products/admin/by-category/${category._id}`);
      if (data.success) setList(data.data);
  ...
  const onFiles = (e) => {
    ...
    if (keepImages.length + newFiles.length + picked.length > 5) { showToast(`Max 5 images. You have ${keepImages.length + newFiles.length} already.`, "error"); return; }
    setNewFiles(p => [...p, ...picked]);
    setNewPreviews(p => [...p, ...picked.map(f => URL.createObjectURL(f))]);
    if (fileRef.current) fileRef.current.value = "";
  };

  const removeNewFile  = (idx) => { setNewFiles(p => p.filter((_, i) => i !== idx)); setNewPreviews(p => p.filter((_, i) => i !== idx)); };
  ...
  const openEdit = (p) => {
    setForm({ name: p.name, label: p.label || "", productCode: p.productCode || "", weight: p.weight || "", price: p.price, isVisible: p.isVisible, isFeatured: p.isFeatured, isBestseller: p.isBestseller });
    setNewFiles([]); setNewPreviews([]); setKeepImages(p.images || []); setEditingId(p._id); setShowForm(true);
  };
  const cancel = () => { setShowForm(false); setEditingId(null); setNewFiles([]); setNewPreviews([]); setKeepImages([]); };

  const onSubmit = async (e) => {
    e.preventDefault();
    if (totalImages === 0) { showToast("Please upload at least 1 image", "error"); return; }
    setSaving(true);
    try {
      const fd = new FormData();
      fd.append("collectionId",   collection._id);
      ...
      const method = editingId ? "put" : "post";
      const { data } = await axiosInstance[method](url, fd);
      if (data.success) { showToast(editingId ? "Product updated!" : "Product created!"); cancel(); load(); }
      else showToast(data.message, "error");
    } catch (err) { showToast(err.response?.data?.message || "Error", "error"); }
    finally { setSaving(false); }
  };
  ...
  const toggleVis = async (id) => {
    try {
      ...
      if (data.success) { setList(p => p.map(x => x._id === id ? { ...x, isVisible: data.data.isVisible } : x)); showToast("Visibility updated"); }
    } catch { showToast("Failed", "error"); }
  };
  ...
  const del = async (id) => {
    ...
    try {
      ...
      if (data.success) { showToast("Product deleted"); load(); }
      else showToast(data.message, "error");
    } catch (err) { showToast(err.response?.data?.message || "Delete failed", "error"); }
  };
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <button onClick={onBack} className="px-3 py-1.5 rounded-xl text-sm border border-amber-200 text-amber-700 hover:bg-amber-50 transition">← Back</button>
        <div>
          <h2 className="text-xl font-normal flex items-center gap-2 flex-wrap" style={{ color: "#2c1a0e" }}>
            <span className="text-sm font-semibold px-2 py-0.5 rounded-lg bg-amber-100 text-amber-800">{collection.name}</span>
            <span className="text-amber-300">›</span>
            <span className='text-sm font-semibold px-2 py-0.5 rounded-lg'>{category.name}</span>
            <span>Products</span>
          </h2>
          <p className="text-xs mt-0.5" style={{ color: "#8b6b4a" }}>{list.length} product{list.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl shadow-md border border-amber-100 p-6 mb-6">
          <h3 className="font-semibold mb-5" style={{ color: "#2c1a0e" }}>{editingId ? "Edit Product" : `New Product in "${category.name}"`}</h3>
          <form onSubmit={onSubmit} className="grid grid-cols-2 gap-4">
            <div className="col-span-2 flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input name="label" value={form.label} onChange={onChange} placeholder="e.g. New Arrival" className={cls} />
            </div>
            <div className="flex flex-col gap-1">
              <Label text="Product Code (SKU)" />
              <input name="productCode" value={form.productCode} onChange={onChange} placeholder="e.g. GLD-CH-001" className={cls} />
            </div>
            <div className="flex flex-col gap-1">
              <Label text="Weight" />
              <input name="weight" value={form.weight} onChange={onChange} placeholder="e.g. 8.5g" className={cls} />
            </div>
            <div className="flex flex-col gap-1">
              <Label text="Price (₹) *" />
              <input name="price" value={form.price} onChange={onChange} required placeholder="e.g. 45000" className={cls} />
            </div>

            <div className="col-span-2 flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <Label text={`Product Images (${totalImages}/5)`} />
                {totalImages >= 5 && <span className="text-xs text-red-400 font-medium">Max 5 reached</span>}
              </div>
              {keepImages.length > 0 && (
                <div>
                  <p className="text-xs text-amber-600 mb-1">Saved images — hover &amp; click × to remove</p>
                  <div className="flex gap-2 flex-wrap">
                    {keepImages.map((url, i) => (
                      <div key={i} className="relative group">
                        <img src={url} alt="" className="w-20 h-20 rounded-xl object-cover border border-amber-200" />
                        <button type="button" onClick={() => removeExisting(url)} className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-red-500 text-white text-xs font-bold opacity-0 group-hover:opacity-100 transition flex items-center justify-center">×</button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {newPreviews.length > 0 && (
                <div>
                  <p className="text-xs text-green-600 mb-1">New images to upload</p>
                  <div className="flex gap-2 flex-wrap">
                    {newPreviews.map((src, i) => (
                      <div key={i} className="relative group">
                        <img src={src} alt="" className="w-20 h-20 rounded-xl object-cover border border-green-200" />
                        <button type="button" onClick={() => removeNewFile(i)} className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-red-500 text-white text-xs font-bold opacity-0 group-hover:opacity-100 transition flex items-center justify-center">×</button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {totalImages < 5 && (
                <label htmlFor="prod-imgs" className="flex flex-col items-center justify-center border-2 border-dashed border-amber-200 rounded-xl py-5 cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition">
                  ...
                  <span className="text-xs text-amber-400 mt-0.5">JPG, PNG, WEBP · max 5 MB each · {5 - totalImages} slot{5 - totalImages !== 1 ? "s" : ""} remaining</span>
                </label>
              )}
            ...
            <div className="col-span-2 flex flex-wrap gap-6 pt-1">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="isVisible" checked={form.isVisible} onChange={onChange} className="w-4 h-4 accent-amber-700" />
                <span className="text-sm text-amber-900">Visible on storefront</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="isFeatured" checked={form.isFeatured} onChange={onChange} className="w-4 h-4 accent-amber-700" />
                <span className="text-sm text-amber-900">⭐ Featured</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="isBestseller" checked={form.isBestseller} onChange={onChange} className="w-4 h-4 accent-amber-700" />
                <span className="text-sm text-amber-900">🔥 Bestseller</span>
              </label>
            </div>
          ...
          {list.map(prod => (
            <div key={prod._id} className={`bg-white rounded-2xl shadow-sm border border-amber-100 p-5 flex gap-4 ${!prod.isVisible ? "opacity-60" : ""}`}>
              <div className="flex-shrink-0">
                {prod.images?.[0]
                  ? <img src={prod.images[0]} alt="" className="w-24 h-24 rounded-xl object-cover border border-amber-100" />
                  : <div className="w-24 h-24 rounded-xl bg-amber-50 flex items-center justify-center text-4xl">💍</div>
                }
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {prod.isBestseller && <span className="px-2 py-0.5 rounded-full text-xs bg-orange-100 text-orange-600 font-semibold">🔥 Bestseller</span>}
                    {prod.isFeatured   && <span className="px-2 py-0.5 rounded-full text-xs bg-yellow-100 text-yellow-700 font-semibold">⭐ Featured</span>}
              ...
              {prod.images?.length > 1 && (
                <div className="flex-shrink-0 flex flex-col gap-1 justify-start">
                  {prod.images.slice(1, 5).map((img, i) => (
                    <img key={i} src={img} alt="" className="w-10 h-10 rounded-lg object-cover border border-amber-100" />
                  ))}
                </div>
              )}
            </div>
...
export default function AdminPanel() {
  ...
  const goCollections = () => { setView("collections"); setSelCollection(null); setSelCategory(null); };
  const goCategories  = (col) => { setSelCollection(col); setView("categories"); };
  const goProducts    = (cat) => { setSelCategory(cat); setView("products"); };
  const backToCats    = () => { setView("categories"); setSelCategory(null); };
    ...
    <div className="min-h-screen px-6 py-10" style={{ background: "#faf8f5", fontFamily: "'Georgia', serif" }}>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-2 mb-8 flex-wrap">
          ...
          {selCategory && (<><span className="text-amber-300">›</span><span className="px-3 py-1.5 rounded-lg bg-amber-100 text-amber-900 font-semibold text-sm">{selCategory.name}</span></>)}
        </div>

        {view === "collections" && <CollectionsPanel onSelect={goCategories} showToast={showToast} />}
        {view === "categories" && selCollection && <CategoriesPanel collection={selCollection} onSelect={goProducts} onBack={goCollections} showToast={showToast} />}
        {view === "products" && selCollection && selCategory && <ProductsPanel collection={selCollection} category={selCategory} onBack={backToCats} showToast={showToast} />}
      </div>
    </div>
  );
}